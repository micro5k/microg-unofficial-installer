...
cp -rf "${SCRIPT_DIR}/"/LICENSE* "${TEMP_DIR}/zip-content/" || ui_error 'Failed to copy the license to the temp dir'
...
java -Duser.timezone=GMT -Dzip.encoding=Cp437 -Djava.io.tmpdir="${TEMP_DIR}/zipsign" -jar "${SCRIPT_DIR}/tools/zipsigner.jar" "${TEMP_DIR}/flashable.zip" "${TEMP_DIR}/${FILENAME}.zip" || ui_error 'Failed signing and zipaligning'
