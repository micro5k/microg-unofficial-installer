...
if ! "${ENV_RESETTED:-false}"; then
  ...
  exec env -i ENV_RESETTED=true THIS_SCRIPT="${THIS_SCRIPT}" OUR_TEMP_DIR="${OUR_TEMP_DIR}" PATH="${PATH}" bash "${THIS_SCRIPT}" "$@"
  fail_with_msg 'Code should never reach this point'
fi
unset ENV_RESETTED
